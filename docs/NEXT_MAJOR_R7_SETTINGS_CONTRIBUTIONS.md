...
恢复内核不承载 Blender、插件、排除规则、历史记录等普通业务设置，也不接受第三方任意 React/HTML 注入。
...
普通可安装组件优先使用 schema 设置：布尔值、数字、字符串、路径、枚举、列表和分组由宿主统一渲染。只有随宿主登记的受信内置实现可以使用 renderer ID，组件包不能在主进程中直接注入 React、HTML 或脚本。
