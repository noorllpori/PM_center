...
- [x] 停用完成后任务显示“已取消”，模块资源数为 0；
- [x] 停用后新建任务、运行脚本、pip/venv 操作和插件动作返回模块停用提示；
- [x] 停用后任务历史、脚本、venv、插件设置和插件目录仍存在；
- [x] 重新启用后 Python 环境、任务中心和旧插件恢复使用；
- [x] 连续启停 5 次，无残留 Python、pip 或插件子进程，资源计数不增长；
- [x] 原有渲染队列和 Blender Worker 不受本模块停用影响。

2026-08-04 用户确认人工验证通过，本分项转为 `accepted`。
