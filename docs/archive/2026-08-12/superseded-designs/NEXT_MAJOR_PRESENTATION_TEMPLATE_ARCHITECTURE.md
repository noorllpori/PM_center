...
- R10-P1 已定义版本化 `template.json`、模板类型、资源路径与白名单检查；外部模板只能是无 Capability 的 `data-pack`，安装暂存区会拒绝脚本、内联事件、远程/绝对资源、CSS 外链和缺少 Shell 恢复节点的包；
- R10-P2 已实现无权限 sandbox iframe 预览：不开放脚本、网络、媒体、表单或 Tauri API；模板包路径不暴露给前端。外部 HTML 目前只用于隔离预览，不能绕过 React 宿主直接接管主窗口；真正切换 Shell 仍须走受控插槽树和原子回退，不能使用 `dangerouslySetInnerHTML`。
- R10-P3 已实现装配编辑器的 Shell 选择、组件启用、Profile 引用校验和卸载前引用保护；模板资源重新载入时会再次进行静态净化。
