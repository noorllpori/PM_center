...
- [ ] 设置页显示 `builtin.render-center` 为运行中，资源数为 1；
- [ ] 启动一个常驻 Worker、多开为 2 的真实任务，确认渐进启动、性能图和帧进度正常；
- [ ] 渲染中停用“渲染中心”，确认两个 Blender 进程树均退出；
- [ ] 停用后作业为“已暂停”，运行帧回到等待，失败数和尝试次数没有增加；
- [ ] 已完成图像、预设、日志和任务历史仍存在且可读取；
- [ ] 停用后开始队列、继续作业、场景检查和视频打包均返回模块停用提示；
- [ ] 重新启用后队列保持暂停，不会自行启动 Blender；
- [ ] 手动点击继续后从未完成帧继续，不重复覆盖有效完成帧；
- [ ] 在逐帧兼容模式、前后置脚本和 FFmpeg 打包期间分别停用，相关进程均能结束且临时文件被清理；
- [ ] 模拟多开驱动崩溃后仍能降为单 Worker，生命周期停用不改变该回退语义；
- [ ] 连续启停 5 次，资源计数不增长，任务管理器无 Blender/Python/FFmpeg 残留；
- [ ] 退出应用时运行任务被安全暂停，不留下孤儿进程，重启后必须手动继续。
