...
pub(crate) fn release_project_streams(project_path: &str) {
    ...
    release_streams_matching(|stream| {
        stream.project_root.as_ref().is_some_and(|root| {
            crate::tree_cache::normalize_path_key(&root.to_string_lossy()) == project_key
        })
    });
}
...
pub(crate) fn required_capabilities_for_command(
    ...
    let mut capabilities = match command {
        ...
        "file.write" | "directory.create" | "stream.open-write" => {
            one_location(LocationAccess::Write)?
        }
        ...
        "batch.execute" => {
            ...
            for operation in operations {
                ...
                all.extend(required_capabilities_for_command(
                    nested_command,
                    &nested_input,
                )?);
            }
        ...
        "external.list-grants" | "external.revoke-grant" => {
            vec![Capability::FilesystemExternalRead]
        }
...
async fn execute_command(
    ...
    match command {
        ...
        "entry.stat" => stat_entry(
            request,
            current_project_root,
            component_root,
            caller_component_id,
        ),
        "entry.exists" => exists_entry(
            request,
            current_project_root,
            component_root,
            caller_component_id,
        ),
        "entry.search" => search_entries(
            request,
            current_project_root,
            component_root,
            caller_component_id,
        ...
        "stream.open-read" => stream_open_read(
            request,
            current_project_root,
            component_root,
            caller_component_id,
        ...
        "stream.open-write" => stream_open_write(
            request,
            current_project_root,
            component_root,
            caller_component_id,
        ...
        "external.revoke-grant" => {
            revoke_external_grant(component_root, request, caller_component_id)
        }
...
fn list_directory(
    ...
    let location = request
        .get("location")
        .is_some()
        .then(|| required_location(request, "location"))
        .transpose()?;
    let resolved = if let Some(location) = location {
        ...
        ResolvedLocation {
            path: resolve_project_path(root, relative, false)?,
    ...
    let cached = if source != "disk" && resolved.project_root.is_some() {
        let root = resolved
            .project_root
            .as_deref()
            .expect("project root checked");
        crate::tree_cache::get_or_create_project_cache(&root.to_string_lossy())
            .ok()
            .and_then(|cache| {
                cache
                    .get_directory_entries(&directory.to_string_lossy())
                    .ok()
            })
    } else {
        None
    ...
    let entries = if let Some(entries) = cached {
        ...
        let mut entries = fs::read_dir(&directory)
            ...
            .filter_map(|entry| {
                entry_metadata_value(resolved.project_root.as_deref(), &entry.path()).ok()
            })
            .collect::<Vec<_>>();
    ...
    let results = WalkDir::new(&root)
        ...
        .filter(|entry| {
            ...
            let kind_matches = kind.is_none_or(|expected| {
                (expected == "directory" && entry.file_type().is_dir())
                    || (expected == "file" && entry.file_type().is_file())
            });
            (query.is_empty()
                || name.contains(&query)
...
fn read_file(
    ...
    if !resolved.path.is_file() {
        return Err(protocol_error("读取目标不是文件"));
    }
    ...
    let length = request
        ...
        .clamp(1, MAX_CONTENT_BYTES as u64) as usize;
    let mut file =
        fs::File::open(&resolved.path).map_err(|error| io_error("打开文件失败", error))?;
    file.seek(SeekFrom::Start(offset))
        .map_err(|error| io_error("定位文件失败", error))?;
    let mut bytes = vec![0; length];
    let count = file
        .read(&mut bytes)
        .map_err(|error| io_error("读取文件失败", error))?;
    ...
    let data = if encoding == "text" {
        String::from_utf8(bytes)
            .map(Value::String)
            .map_err(|_| protocol_error("文件内容不是 UTF-8 文本"))?
    } else {
        Value::String(base64::engine::general_purpose::STANDARD.encode(bytes))
    };
    Ok(
        json!({"data": data, "encoding": encoding, "offset": offset, "bytesRead": count, "eof": count < length}),
    )
}
...
fn write_file(
    ...
    if mode == "append" {
        ...
        let mut file = fs::OpenOptions::new()
            ...
            .append(true)
            .open(&resolved.path)
            .map_err(|error| io_error("追加文件失败", error))?;
        file.write_all(&bytes)
            .map_err(|error| io_error("写入文件失败", error))?;
    } else {
...
async fn copy_entry(
    ...
    let source = resolve_location(
        &required_location(request, "source")?,
        current_root,
        component_root,
        caller,
        false,
    ...
    let target = resolve_location(
        &required_location(request, "target")?,
        current_root,
        component_root,
        caller,
        true,
    ...
    let target = resolve_conflict_target(
        target.path,
        request
            .get("conflict")
            .and_then(Value::as_str)
    ...
    if source.path == target {
        return Err(protocol_error("源和目标不能相同"));
    }
...
async fn move_entry(
    ...
    let parent = target
        .path
        .parent()
        .ok_or_else(|| protocol_error("移动目标缺少父目录"))?
        .to_path_buf();
    let name = target
        .path
        .file_name()
    ...
    let conflict = request
        ...
        .and_then(Value::as_str)
        .unwrap_or("error");
    let moved = crate::fs::move_path_with_strategy(source.path.clone(), parent, conflict, name)
        .await
        .map_err(protocol_error)?;
    mark_project_dirty(source.project_root.as_deref());
    mark_project_dirty(target_root.as_deref());
...
async fn rename_entry(
    ...
    let name = request
        ...
        .and_then(Value::as_str)
        .filter(|value| !value.trim().is_empty())
        .ok_or_else(|| protocol_error("重命名缺少 name"))?;
    if Path::new(name).components().count() != 1 {
        return Err(protocol_error("文件名不能包含目录分隔符"));
    }
    let target = source
        .path
        .parent()
        .ok_or_else(|| protocol_error("重命名目标无父目录"))?
        .join(name);
    let target = resolve_conflict_target(
        target,
        request
...
async fn delete_entry(
    ...
    if permanent {
        if resolved.path.is_dir() {
            tokio::fs::remove_dir_all(&resolved.path)
                .await
                .map_err(|error| io_error("永久删除目录失败", error))?;
        } else {
            tokio::fs::remove_file(&resolved.path)
                .await
                .map_err(|error| io_error("永久删除文件失败", error))?;
        }
    } else {
        crate::fs::delete_paths(vec![resolved.path.to_string_lossy().to_string()])
            .await
            .map_err(protocol_error)?;
    }
    mark_project_dirty(resolved.project_root.as_deref());
...
async fn batch_execute(
    ...
    let operations = request
        .get("operations")
        .and_then(Value::as_array)
        .ok_or_else(|| protocol_error("批量操作缺少 operations"))?;
    if operations.is_empty() || operations.len() > MAX_BATCH_OPERATIONS {
        return Err(protocol_error("批量操作数量必须在 1 到 100 之间"));
    }
    ...
    for operation in operations {
        let command = operation
            .get("command")
            .and_then(Value::as_str)
            .ok_or_else(|| protocol_error("批量项目缺少 command"))?;
        if command == "batch.execute" {
            return Err(protocol_error("批量操作不能递归调用 batch.execute"));
        }
...
fn stream_open_read(
    ...
    let size = fs::metadata(&resolved.path)
        .map_err(|error| io_error("读取文件元数据失败", error))?
        .len();
    let id = Uuid::new_v4().to_string();
    STREAMS
        .lock()
        .map_err(|_| protocol_error("流状态不可用"))?
        .insert(
            id.clone(),
            FileStream {
                ...
                grant_id: (location.space == "external")
                    .then(|| location.grant_id)
                    .flatten(),
                access: LocationAccess::Read,
                offset: 0,
...
fn stream_read(
    ...
    let length = request
        ...
        .clamp(1, MAX_STREAM_CHUNK_BYTES as u64) as usize;
    let mut streams = STREAMS.lock().map_err(|_| protocol_error("流状态不可用"))?;
    let stream = streams
        .get_mut(id)
        .ok_or_else(|| protocol_error("读取流不存在或已过期"))?;
    if stream.component_id != caller || stream.mode != "read" {
        return Err(protocol_error("读取流不属于当前组件"));
    }
    validate_stream_scope(stream, component_root, caller)?;
    let mut file =
        fs::File::open(&stream.path).map_err(|error| io_error("打开读取流失败", error))?;
    file.seek(SeekFrom::Start(stream.offset))
        .map_err(|error| io_error("定位读取流失败", error))?;
    let mut buffer = vec![0; length];
    let count = file
        .read(&mut buffer)
        .map_err(|error| io_error("读取流失败", error))?;
    buffer.truncate(count);
    stream.offset += count as u64;
    stream.expires_at = Utc::now().timestamp_millis() + STREAM_TTL_MS;
    Ok(
        json!({"dataBase64": base64::engine::general_purpose::STANDARD.encode(buffer), "bytesRead": count, "offset": stream.offset, "eof": count < length}),
    )
}

fn stream_open_write(
    request: &Value,
    current_root: Option<&Path>,
    component_root: &Path,
    caller: &str,
    ...
    let parent = resolved
        .path
        .parent()
        .ok_or_else(|| protocol_error("写入目标缺少父目录"))?;
    fs::create_dir_all(parent).map_err(|error| io_error("创建父目录失败", error))?;
...
fn stream_write(
    ...
    let bytes = request
        .get("dataBase64")
        .and_then(Value::as_str)
        .ok_or_else(|| protocol_error("写入流缺少 dataBase64"))?;
    let bytes = base64::engine::general_purpose::STANDARD
        .decode(bytes)
        .map_err(|_| protocol_error("dataBase64 无效"))?;
    if bytes.len() > MAX_STREAM_CHUNK_BYTES {
        return Err(protocol_error("流分块不能超过 1 MiB"));
    }
    let mut streams = STREAMS.lock().map_err(|_| protocol_error("流状态不可用"))?;
    let stream = streams
        .get_mut(id)
        .ok_or_else(|| protocol_error("写入流不存在或已过期"))?;
    if stream.component_id != caller || stream.mode != "write" {
        return Err(protocol_error("写入流不属于当前组件"));
    }
    validate_stream_scope(stream, component_root, caller)?;
    let temporary = stream
        .temporary_path
        .as_ref()
        .ok_or_else(|| protocol_error("写入流临时文件缺失"))?;
    let mut file = fs::OpenOptions::new()
        .append(true)
        .open(temporary)
        .map_err(|error| io_error("打开写入流失败", error))?;
    file.write_all(&bytes)
        .map_err(|error| io_error("写入流失败", error))?;
...
fn stream_commit(
    ...
    let pending = streams
        ...
        .cloned()
        .ok_or_else(|| protocol_error("写入流不存在或已过期"))?;
    if pending.component_id != caller || pending.mode != "write" {
        return Err(protocol_error("写入流不属于当前组件"));
    }
    validate_stream_scope(&pending, component_root, caller)?;
    let stream = streams
        .remove(id)
        .expect("stream remains registered after validation");
    let temporary = stream
        .temporary_path
        .ok_or_else(|| protocol_error("写入流临时文件缺失"))?;
    if stream.path.exists() {
        fs::remove_file(&stream.path).map_err(|error| io_error("替换目标文件失败", error))?;
    }
...
fn stream_abort(request: &Value, caller: &str) -> Result<Value, ComponentRuntimeError> {
    ...
    let stream = STREAMS
        ...
        .map_err(|_| protocol_error("流状态不可用"))?
        .remove(id)
        .ok_or_else(|| protocol_error("流不存在或已过期"))?;
    if stream.component_id != caller {
        return Err(protocol_error("流不属于当前组件"));
    }
    if let Some(temporary) = stream.temporary_path {
        let _ = fs::remove_file(temporary);
    }
...
fn create_external_grant(
    ...
    if !root.is_absolute() || !root.is_dir() {
        return Err(protocol_error("外部授权目录必须是存在的绝对目录"));
    }
        ...
        .unwrap_or("once")
    {
        ...
        _ => {
            return Err(protocol_error(
                "外部路径授权 lifetime 只支持 once、session 或 persistent",
            ))
        }
    };
    let access = request
    ...
    if !matches!(access, "read" | "write" | "read-write") {
        return Err(protocol_error(
            "外部路径授权 access 只支持 read、write 或 read-write",
        ));
    }
    let grant = ExternalPathGrant {
        ...
        expires_at: if lifetime == GrantLifetime::Once {
    ...
    match lifetime {
        GrantLifetime::Persistent => {
            ...
            save_persistent_grants(component_root, &grants)?;
        }
        _ => {
            SESSION_GRANTS
                .lock()
                .map_err(|_| protocol_error("授权状态不可用"))?
...
fn list_external_grants(
    ...
    grants.extend(
        SESSION_GRANTS
            ...
            .map_err(|_| protocol_error("授权状态不可用"))?
            .values()
            .filter(|grant| grant.component_id == caller)
            .cloned(),
    );
    grants.retain(|grant| {
        grant.component_id == caller && grant.expires_at.is_none_or(|expires| expires > now)
    });
    Ok(json!({"grants": grants}))
}

fn revoke_external_grant(
    component_root: &Path,
    request: &Value,
    caller: &str,
) -> Result<Value, ComponentRuntimeError> {
...
fn cache_status(current_root: Option<&Path>) -> Result<Value, ComponentRuntimeError> {
    let root = require_project_root(current_root)?;
    let cache = crate::tree_cache::get_or_create_project_cache(&root.to_string_lossy())
        .map_err(protocol_error)?;
    Ok(
        json!({"projectId": project_id(root), "dirtyDirectories": cache.count_dirty_dirs().map_err(protocol_error)?, "registered": crate::tree_cache::project_cache_count()}),
    )
}
...
fn cache_query(
    ...
    let cache = crate::tree_cache::get_or_create_project_cache(&root.to_string_lossy())
        .map_err(protocol_error)?;
    let entries = cache
        .get_directory_entries(&dir.to_string_lossy())
        .map_err(protocol_error)?;
    Ok(
        json!({"entries": entries.into_iter().map(|entry| json!({"path": project_relative(root, Path::new(&entry.path)), "name": entry.name, "kind": if entry.is_dir { "directory" } else { "file" }, "sizeBytes": entry.size})).collect::<Vec<_>>()}),
    )
}
...
fn cache_refresh(
    ...
    let cache = crate::tree_cache::get_or_create_project_cache(&root.to_string_lossy())
        .map_err(protocol_error)?;
    let entries = crate::tree_cache::scan_directory_entries_from_disk(
        &root.to_string_lossy(),
        &dir.to_string_lossy(),
    )
    .map_err(protocol_error)?;
    cache
        .replace_directory_entries(&dir.to_string_lossy(), &entries)
        .map_err(protocol_error)?;
...
fn cache_rebuild(current_root: Option<&Path>) -> Result<Value, ComponentRuntimeError> {
    let root = require_project_root(current_root)?;
    crate::tree_cache::rebuild_project_tree_cache(&root.to_string_lossy())
        .map_err(protocol_error)?;
...
fn watcher_status(current_root: Option<&Path>) -> Result<Value, ComponentRuntimeError> {
    let root = require_project_root(current_root)?;
    Ok(
        json!({"projectId": project_id(root), "activeProject": crate::watcher::get_active_project_path(), "watcherActive": crate::watcher::get_active_project_path().as_deref() == Some(&root.to_string_lossy())}),
    )
}
...
fn required_location(request: &Value, key: &str) -> Result<FileLocation, ComponentRuntimeError> {
    serde_json::from_value(
        request
            .get(key)
            .cloned()
            .ok_or_else(|| protocol_error(format!("缺少 {key}")))?,
    )
    .map_err(|error| protocol_error(format!("{key} 无效: {error}")))
}

fn required_string<'a>(request: &'a Value, key: &str) -> Result<&'a str, ComponentRuntimeError> {
    request
        .get(key)
        .and_then(Value::as_str)
        .filter(|value| !value.trim().is_empty())
...
fn resolve_location(
    ...
    allow_missing: bool,
    access: LocationAccess,
) -> Result<ResolvedLocation, ComponentRuntimeError> {
    match location.space.as_str() {
        ...
        "external" => {
            let grant_id = location
                .grant_id
                .as_deref()
                .ok_or_else(|| protocol_error("外部路径缺少 grantId"))?;
            let grant = find_grant(component_root, grant_id, caller)?;
...
fn resolve_project_context_root(
    ...
    if let Some(project_id) = requested_project_id {
        if current
            .as_deref()
            .is_some_and(|root| project_id_for(root) == project_id)
        {
            return current
...
fn ensure_grant_access(
    grant: &ExternalPathGrant,
    requested: LocationAccess,
) -> Result<(), ComponentRuntimeError> {
    ...
    if permitted {
        ...
        let action = match requested {
            LocationAccess::Read => "读取",
            LocationAccess::Write => "写入",
        };
...
fn require_project_root(root: Option<&Path>) -> Result<&Path, ComponentRuntimeError> {
    ...
    if !root.is_dir() {
        return Err(protocol_error("项目目录已关闭或不可访问"));
    }
    crate::project_resources::ensure_project_access(&root.to_string_lossy())
        .map_err(protocol_error)?;
    Ok(root)
}
...
fn resolve_project_path(
    ...
    {
        return Err(protocol_error("项目路径必须是安全相对路径"));
    }
    if relative
        .components()
        .any(|component| component.as_os_str().eq_ignore_ascii_case(".pm_center"))
    {
        return Err(protocol_error("组件不能直接访问 .pm_center"));
    }
...
fn resolve_external_path(
    ...
    if !path.is_absolute() {
        return Err(protocol_error("外部路径必须是绝对路径"));
    }
...
fn resolve_contained(
    ...
    let canonical_root =
        fs::canonicalize(root).map_err(|error| io_error("解析授权根目录失败", error))?;
    let candidate = if path.exists() {
        fs::canonicalize(path).map_err(|error| io_error("解析路径失败", error))?
    } else if allow_missing {
        let mut ancestor = path;
        while !ancestor.exists() {
            ancestor = ancestor
                .parent()
                .ok_or_else(|| protocol_error("路径没有有效父目录"))?;
        }
        let canonical_ancestor =
            fs::canonicalize(ancestor).map_err(|error| io_error("解析父目录失败", error))?;
        let suffix = path
            .strip_prefix(ancestor)
            .map_err(|_| protocol_error("路径解析失败"))?;
        canonical_ancestor.join(suffix)
    } else {
        return Err(protocol_error("路径不存在"));
    };
    if !candidate.starts_with(&canonical_root) {
        return Err(protocol_error("路径超出项目或外部授权范围"));
    }
    Ok(candidate)
}
...
fn find_grant(
    ...
    id: &str,
    caller: &str,
) -> Result<ExternalPathGrant, ComponentRuntimeError> {
    let now = Utc::now().timestamp_millis();
    if let Some(grant) = SESSION_GRANTS
        .lock()
        .map_err(|_| protocol_error("授权状态不可用"))?
        .get(id)
        .cloned()
    {
        if grant.component_id == caller && grant.expires_at.is_none_or(|expires| expires > now) {
            return Ok(grant);
        }
    }
    load_persistent_grants(component_root)?
        .into_iter()
        .find(|grant| grant.id == id && grant.component_id == caller)
        .ok_or_else(|| protocol_error("外部路径授权不存在、已过期或不属于当前组件"))
}
...
fn load_persistent_grants(root: &Path) -> Result<Vec<ExternalPathGrant>, ComponentRuntimeError> {
    ...
    serde_json::from_slice(
        &fs::read(path).map_err(|error| io_error("读取外部路径授权失败", error))?,
    )
    .map_err(|error| protocol_error(format!("外部路径授权格式无效: {error}")))
}

fn save_persistent_grants(
    root: &Path,
    grants: &[ExternalPathGrant],
) -> Result<(), ComponentRuntimeError> {
    fs::create_dir_all(root).map_err(|error| io_error("创建授权目录失败", error))?;
    atomic_write(
        &root.join("file-operation-grants.json"),
        &serde_json::to_vec_pretty(grants).map_err(|error| protocol_error(error.to_string()))?,
    )
}
...
fn decode_request_bytes(request: &Value) -> Result<Vec<u8>, ComponentRuntimeError> {
    ...
    let data = request
        .get("data")
        .and_then(Value::as_str)
        .ok_or_else(|| protocol_error("写入操作缺少 data"))?;
    if encoding == "text" {
        Ok(data.as_bytes().to_vec())
    } else {
        base64::engine::general_purpose::STANDARD
            .decode(data)
            .map_err(|_| protocol_error("data 不是有效 Base64"))
    }
}
...
fn atomic_write(path: &Path, bytes: &[u8]) -> Result<(), ComponentRuntimeError> {
    ...
    let result = (|| -> Result<(), ComponentRuntimeError> {
        let mut file =
            fs::File::create(&temporary).map_err(|error| io_error("创建临时文件失败", error))?;
        file.write_all(bytes)
            .map_err(|error| io_error("写入临时文件失败", error))?;
        file.sync_all()
            .map_err(|error| io_error("同步临时文件失败", error))?;
        if path.exists() {
            fs::remove_file(path).map_err(|error| io_error("替换目标文件失败", error))?;
        }
...
fn resolve_conflict_target(
    ...
    match conflict {
        "overwrite" => {
            if target.is_dir() {
                fs::remove_dir_all(&target).map_err(|error| io_error("覆盖目标目录失败", error))?;
            } else {
                fs::remove_file(&target).map_err(|error| io_error("覆盖目标文件失败", error))?;
            }
            Ok(target)
        }
        "rename" => {
            let parent = target
                .parent()
                .ok_or_else(|| protocol_error("冲突目标缺少父目录"))?
                .to_path_buf();
            let name = target
                .file_name()
                .and_then(|value| value.to_str())
            ...
            for index in 1..10000 {
                let candidate = parent.join(if let Some(extension) = extension {
            ...
            Err(protocol_error("无法生成不冲突的文件名"))
        }
...
fn hash_path(path: &Path) -> Result<String, ComponentRuntimeError> {
    let mut file = fs::File::open(path).map_err(|error| io_error("读取文件失败", error))?;
    let mut hasher = blake3::Hasher::new();
    let mut buffer = [0u8; 64 * 1024];
    loop {
        let count = file
...
fn mark_project_dirty(root: Option<&Path>) {
    if let Some(root) = root {
        crate::tree_cache::mark_project_tree_dirty(&root.to_string_lossy());
    }
}
fn purge_expired_streams() {
    let now = Utc::now().timestamp_millis();
    if let Ok(mut streams) = STREAMS.lock() {
        let expired = streams
            .iter()
            .filter(|(_, stream)| stream.expires_at <= now)
            .map(|(id, stream)| (id.clone(), stream.temporary_path.clone()))
            .collect::<Vec<_>>();
        for (id, temporary) in expired {
            streams.remove(&id);
            if let Some(temporary) = temporary {
                let _ = fs::remove_file(temporary);
            }
...
fn protocol_error(message: impl Into<String>) -> ComponentRuntimeError {
    ComponentRuntimeError::new(ComponentRuntimeErrorCode::ComponentProtocolError, message)
}
fn io_error(label: &str, error: std::io::Error) -> ComponentRuntimeError {
